...
<script>
	...
	export default {
		...
		onLoad(options){
			_self = this;
			_self.checkLogin(2);
			_self.headermsg = '体温查询,Temperature';
		},
		onReady(){
			_self.show();
		},
		methods:{
			...
			bindclick(){
				...
				if(com_id*1 > 0 && sid*1 > 0 && cat_id*1 > 0 && d != '' ){
					_self.navigateTo('statisticstwresult?com_id='+com_id+"&d="+d+"&sid="+sid+"&cat_id="+cat_id);
				}else{
					uni.showToast({
						title: '请选择相关信息',
						icon: 'none',
			...
			getData(data){
				...
				list2.push("==请选择月份==");
				idlist2.push(0);
				let vlist = _self.GetStudyMonth();
				for (var i = 0; i < vlist.length; i++) {
					list2.push(vlist[i]);
				}
				_self.dateList = list2;
			...
			categoryPickerChange:function(e){
				...
				let ret = _self.getUserInfo();
				this.sendRequest({
				    url : this.GetAllStudentsByComidCatidUrl,
				    method : _self.Method,
				    data : {
						"guid": ret.guid,
						"token":ret.token,
...
<style>		
	.ctitles{
		background:url(../../../static/img/tw.png) 10upx 25upx no-repeat;
